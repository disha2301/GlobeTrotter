...
import { getUserProfile, updateUserScore, resetAllProfiles } from '@/integrations/supabase/client';
...
const Index = () => {
  ...
  const [challengerInfo, setChallengerInfo] = useState<{name: string, score: number} | null>(null);
  ...
  const handleProfileCreated = (newUsername: string) => {
    ...
    const storedScore = localStorage.getItem(`globetrotter_score_${newUsername}`);
    if (storedScore) {
      try {
  ...
  useEffect(() => {
    if (username) {
      localStorage.setItem(`globetrotter_score_${username}`, JSON.stringify(score));
    }
  }, [score, username]);
        ...
        {username ? (
          <>
            <div className="w-full flex flex-col items-center mt-8 sm:mt-12">
              ...
              <div className="w-full flex justify-between items-center mb-6">
                <div className="flex items-center space-x-2">
                  <div className="flex items-center bg-gradient-to-r from-indigo-100 to-purple-100 px-3 py-1 rounded-full border border-indigo-200">
                    <span className="text-sm font-medium text-indigo-700">Player name: {username}</span>
                  </div>
                </div>
                ...
                <div className="flex space-x-2">
                  ...
                  {}
                  {isGameOwner && (
                    <ResetGameButton username={username} onReset={handleGameReset} />
                  )}
                </div>
              </div>
                ...
                <div className="w-full mb-6 bg-gradient-to-r from-amber-100 to-yellow-100 border border-amber-200 rounded-lg p-3 text-center animate-fade-in">
                  <div className="flex items-center justify-center gap-2">
                    <p className="text-amber-800">
                      <strong>{challengerInfo.name}</strong> has challenged you! Their score: <strong>{challengerInfo.score}</strong> correct answers.
                    </p>
                  </div>
                </div>
              )}
              ...
              <ProgressBar correct={score.correct} incorrect={score.incorrect} />
          ...
          <div className="w-full flex flex-col items-center mt-8 sm:mt-12 animate-fade-in">
            ...
            {challengerInfo && (
              <div className="mt-4 p-4 bg-gradient-to-r from-amber-100 to-yellow-100 border border-amber-200 rounded-lg text-center max-w-md">
                ...
                <p className="text-sm mt-2 text-amber-700">Create a profile to accept the challenge!</p>
              </div>
            )}
          </div>
        )}
